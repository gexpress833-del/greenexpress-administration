<section>
    <header class="mb-5">
        <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100">Modifier le mot de passe</h2>
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Utilisez un mot de passe long et aléatoire pour sécuriser votre compte.</p>
    </header>

    <form method="post" action="{{ route('password.update') }}" class="space-y-5" x-data="{ loading: false, showCurrent: false, showNew: false, showConfirm: false }" @submit="loading = true">
        @csrf
        @method('put')
        ...
        <div class="flex items-center gap-4">
            <button type="submit" :disabled="loading" class="bg-green-600 hover:bg-green-700 disabled:bg-green-500 text-white font-semibold py-2 px-4 rounded-lg transition flex items-center gap-2 disabled:cursor-not-allowed">
                ...
                <span x-text="loading ? 'Mise à jour...' : 'Mettre à jour'">Mettre à jour</span>
            </button>
