<x-app-layout>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 mb-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div class="bg-green-50 dark:bg-green-900/20 rounded-lg p-4">
                ...
                <p class="text-xs text-gray-400 dark:text-gray-500">{{ number_format($totalValueFc ?? 0, 0, '.', '') }} FC</p>
            </div>
            <div class="bg-yellow-50 dark:bg-yellow-900/20 rounded-lg p-4">
                ...
                <p class="text-xs text-gray-400 dark:text-gray-500">{{ number_format($totalWithdrawnFc ?? 0, 0, '.', '') }} FC</p>
            </div>
            <div class="bg-sky-50 dark:bg-sky-900/20 rounded-lg p-4">
                ...
                <p class="text-xs text-gray-400 dark:text-gray-500">{{ number_format($availableFc ?? 0, 0, '.', '') }} FC</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 mb-8">
        ...
        @else
            <form method="POST" action="{{ route('agent.withdrawals.store') }}" class="flex flex-col gap-4" x-data="{ loading: false, currency: 'usd', rate: {{ $exchangeRate }} }" @submit="loading = true">
                @csrf
