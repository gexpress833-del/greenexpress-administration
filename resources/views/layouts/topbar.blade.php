        ...
        <div class="flex items-center gap-2 sm:gap-4 ml-auto">
            ...
            @if(auth()->user()->isAgent())
                <a href="{{ route('agent.points.index') }}" class="flex items-center gap-1 sm:gap-1.5 bg-blue-50 dark:bg-blue-900/30 hover:bg-blue-100 dark:hover:bg-blue-900/50 border border-blue-200 dark:border-blue-800 rounded-full pl-1.5 pr-2 sm:pl-2 sm:pr-3 py-1 transition" title="Voir mon solde de points">
                    ...
                    <span class="text-[10px] sm:text-xs font-bold text-blue-700 dark:text-blue-300 leading-none">{{ app(\App\Services\PointService::class)->getAvailablePoints(auth()->id()) }}<span class="hidden sm:inline"> pts</span></span>
                </a>
            @endif
            ...
            @if(auth()->user()->isLivreur())
                <a href="{{ route('livreur.points.index') }}" class="flex items-center gap-1 sm:gap-1.5 bg-amber-50 dark:bg-amber-900/30 hover:bg-amber-100 dark:hover:bg-amber-900/50 border border-amber-200 dark:border-amber-800 rounded-full pl-1.5 pr-2 sm:pl-2 sm:pr-3 py-1 transition" title="Voir mes points livreur">
                    ...
                    <span class="text-[10px] sm:text-xs font-bold text-amber-700 dark:text-amber-300 leading-none">{{ app(\App\Services\LivreurPointService::class)->getAvailablePoints(auth()->id()) }}<span class="hidden sm:inline"> pts</span></span>
                </a>
            @endif
