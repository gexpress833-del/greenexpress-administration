                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-3">
                            <form method="POST" action="{{ route('admin.meals.toggle-status', $meal) }}" class="inline">
                                @csrf
                                <button type="submit" class="text-sm font-medium {{ $meal->status === 'available' ? 'text-orange-600 hover:text-orange-800 dark:text-orange-400 dark:hover:text-orange-300' : 'text-green-600 hover:text-green-800 dark:text-green-400 dark:hover:text-green-300' }}">
                                    {{ $meal->status === 'available' ? 'Rendre indisponible' : 'Rendre disponible' }}
                                </button>
                            </form>
