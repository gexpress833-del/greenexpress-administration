        ...
        <form method="POST" action="{{ route('admin.subscription-types.store') }}">
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                ...
                <div>
                    ...
                    <input type="number" name="price_fc" step="1" min="0" value="{{ old('price_fc') }}" class="mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100 shadow-sm focus:border-green-500 focus:ring-green-500">
            ...
            <div class="grid grid-cols-3 gap-4 mb-4">
                ...
                <div>
                    ...
                    <input type="number" name="meals_per_day" min="1" max="3" required value="{{ old('meals_per_day', 1) }}" class="mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100 shadow-sm focus:border-green-500 focus:ring-green-500">
                </div>
                <div>
                                    ...
                                    <div>
                                        ...
                                        <select name="weekly_menu[{{ $week }}][{{ $day }}]" class="block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100 text-sm">
                                            ...
                                            @foreach($meals as $meal)
                                                <option value="{{ $meal->id }}" {{ old("weekly_menu.$week.$day") == $meal->id ? 'selected' : '' }}>{{ $meal->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                @endforeach
