<x-app-layout>
    ...
    <form method="get" action="{{ route('admin.dashboard') }}" class="mb-4 flex flex-col sm:flex-row sm:items-center gap-2">
        <div class="flex items-center gap-2 flex-1">
            ...
            <span class="text-slate-400 text-xs sm:text-sm shrink-0">Ã </span>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div class="rounded-2xl border border-white/10 bg-white/[0.07] p-5 shadow-2xl shadow-black/20 backdrop-blur-2xl">
            <p class="text-sm text-slate-400">Chiffre d'affaires validÃ©</p>
            <p class="text-2xl font-bold text-green-700">$ {{ number_format($kpi['financial']['total_revenue_usd'], 2) }}</p>
            <p class="text-xs text-slate-400 mt-1">PÃ©riode : {{ $kpi['period']['start'] }} - {{ $kpi['period']['end'] }}</p>
        </div>
        <div class="rounded-2xl border border-white/10 bg-white/[0.07] p-5 shadow-2xl shadow-black/20 backdrop-blur-2xl">
            <p class="text-sm text-slate-400">Commandes validÃ©es</p>
            <p class="text-2xl font-bold text-white">{{ $kpi['orders']['validated'] }}</p>
            <p class="text-xs text-slate-500 mt-1">{{ $kpi['orders']['total'] }} total â€” {{ $kpi['orders']['cancelled'] }} annulÃ©es</p>
        </div>
        <div class="rounded-2xl border border-white/10 bg-white/[0.07] p-5 shadow-2xl shadow-black/20 backdrop-blur-2xl">
            <p class="text-sm text-slate-400">Profit estimÃ©</p>
            <p class="text-2xl font-bold text-blue-700">$ {{ number_format($kpi['financial']['profit_estimate'], 2) }}</p>
            <p class="text-xs text-slate-400 mt-1">Marge 25% â€” Commissions dÃ©duites</p>
        </div>
        <div class="rounded-2xl border border-white/10 bg-white/[0.07] p-5 shadow-2xl shadow-black/20 backdrop-blur-2xl">
            <p class="text-sm text-slate-400">Commissions versÃ©es</p>
        ...
        <div class="rounded-2xl border border-white/10 bg-white/[0.07] p-5 shadow-2xl shadow-black/20 backdrop-blur-2xl">
            <p class="text-sm text-slate-400">CoÃ»t moyen livraison</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    ...
                    <tbody class="divide-y divide-white/10">
                        ...
                        @empty
                            <tr><td class="px-4 py-2 text-slate-400" colspan="3">Aucune donnÃ©e</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
        ...
        <div class="rounded-[2rem] border border-white/10 bg-white/[0.07] p-6 shadow-2xl shadow-black/20 backdrop-blur-2xl">
            ...
            @php
                $statusLabels = ['pending' => 'En attente','confirmed' => 'ConfirmÃ©e','preparing' => 'En prÃ©paration','delivering' => 'En livraison','delivered' => 'LivrÃ©e','cancelled' => 'AnnulÃ©e'];
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div class="overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.07] shadow-2xl shadow-black/20 backdrop-blur-2xl">
            <div class="px-6 py-4 border-b border-white/10 flex items-center justify-between">
                <h2 class="text-lg font-semibold text-white">Abonnements rÃ©cents</h2>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div class="overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.07] shadow-2xl shadow-black/20 backdrop-blur-2xl">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    <thead class="bg-slate-950/45"><tr><th class="px-6 py-3 text-left text-slate-400">Nom</th><th class="px-6 py-3 text-left text-slate-400">Commandes validÃ©es</th><th class="px-6 py-3 text-left text-slate-400">TÃ©lÃ©phone</th></tr></thead>
                    <tbody class="divide-y divide-white/10">
                        @forelse($kpi['top_agents'] as $agent)
                            <tr>
                                <td class="px-6 py-3 font-medium">{{ $agent->name }}</td>
