        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="flex flex-col sm:flex-row gap-3">
                @if($order->status === 'confirmed')
                    <form method="POST" action="{{ route('cuisinier.orders.update-status', $order) }}" class="flex-1" x-data="{ loading: false }" @submit="loading = true">
                        @csrf
                        @method('PATCH')
                        <input type="hidden" name="status" value="preparing">
                        <button type="submit" :disabled="loading" class="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-blue-500 text-white font-semibold py-2.5 px-4 rounded-lg transition flex items-center justify-center gap-2 disabled:cursor-not-allowed">
                            ...
                            <span x-text="loading ? 'Mise à jour...' : 'Commencer la préparation'">Commencer la préparation</span>
                        </button>
                    </form>
                @elseif($order->status === 'preparing')
                    <form method="POST" action="{{ route('cuisinier.orders.update-status', $order) }}" class="flex-1" x-data="{ loading: false }" @submit="loading = true">
                        ...
                        <button type="submit" :disabled="loading" class="w-full bg-purple-600 hover:bg-purple-700 disabled:bg-purple-500 text-white font-semibold py-2.5 px-4 rounded-lg transition flex items-center justify-center gap-2 disabled:cursor-not-allowed">
                            <template x-if="loading">
                                <svg class="w-5 h-5 animate-spin" fill="none" viewBox="0 0 24 24">
                                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"/>
                            ...
                            <span x-text="loading ? 'Mise à jour...' : 'Marquer prête pour livraison'">Marquer prête pour livraison</span>
                        </button>
                    </form>
                @endif
