...
Route::middleware(['auth', 'password.changed'])->group(function () {
    ...
    Route::middleware(['role:admin'])->prefix('admin')->name('admin.')->group(function () {
        ...
        Route::resource('meals', \App\Http\Controllers\Admin\MealController::class)->except(['show']);
        ...
        Route::resource('deliveries', \App\Http\Controllers\Admin\DeliveryController::class)->only(['index', 'create', 'store']);
        Route::resource('exchange-rates', \App\Http\Controllers\Admin\ExchangeRateController::class)->only(['index', 'store']);
        Route::resource('commissions', \App\Http\Controllers\Admin\CommissionController::class)->only(['index']);
        Route::resource('withdrawals', \App\Http\Controllers\Admin\WithdrawalController::class)->only(['index', 'update']);
        Route::get('/suspensions', [\App\Http\Controllers\Admin\SubscriptionSuspensionController::class, 'index'])->name('suspensions.index');
        Route::post('/suspensions/{suspension}/accept', [\App\Http\Controllers\Admin\SubscriptionSuspensionController::class, 'accept'])->name('suspensions.accept');
        Route::post('/suspensions/{suspension}/reject', [\App\Http\Controllers\Admin\SubscriptionSuspensionController::class, 'reject'])->name('suspensions.reject');
    ...
    Route::middleware(['role:agent'])->prefix('agent')->name('agent.')->group(function () {
        ...
        Route::get('/receipt/{order}', [\App\Http\Controllers\Agent\ReceiptController::class, 'show'])->name('receipt.show');
        Route::get('/receipt/{order}/pdf', [\App\Http\Controllers\Agent\ReceiptController::class, 'pdf'])->name('receipt.pdf');
