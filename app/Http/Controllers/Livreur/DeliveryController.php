...
class DeliveryController extends Controller
{
    ...
    {
        if ($delivery->livreur_id !== $request->user()->id && $delivery->livreur_id !== null) {
            return redirect()->route('livreur.deliveries.index')
                ->with('error', 'Cette livraison est prise en charge par un autre livreur. Vous ne pouvez pas y accéder.');
        }
        ...
        if (! in_array($delivery->order->status, ['confirmed', 'preparing', 'delivering', 'delivered']) || $delivery->order->admin_validated_at === null) {
            return redirect()->route('livreur.deliveries.index')
                ->with('error', 'Cette commande n\'a pas encore été validée par l\'administrateur. Elle ne peut pas être livrée pour le moment. Veuillez patienter ou contacter un administrateur.');
        }
    ...
    public function assign(Request $request, Delivery $delivery, DeliveryService $deliveryService)
    {
        if ($delivery->livreur_id !== null) {
            return redirect()->route('livreur.deliveries.index')
                ->with('error', 'Cette livraison a déjà été prise en charge par un autre livreur.');
        }
        ...
        if (! in_array($delivery->order->status, ['confirmed', 'preparing']) || $delivery->order->admin_validated_at === null) {
            return redirect()->route('livreur.deliveries.index')
                ->with('error', 'Cette commande n\'a pas encore été validée par l\'administrateur. Vous ne pouvez pas encore la prendre en charge.');
        }
    ...
    public function validateQrForm(Request $request, DeliveryService $deliveryService)
    {
        ...
        if (! $delivery) {
            return redirect()->route('livreur.deliveries.index')
                ->with('error', 'Aucune livraison ne correspond à ce code QR. Vérifiez que vous scannez le bon reçu.');
        }
        ...
        if (! in_array($order->status, ['confirmed', 'preparing', 'delivering', 'delivered']) || $order->admin_validated_at === null) {
            return redirect()->route('livreur.deliveries.index')
                ->with('error', 'Cette commande n\'a pas encore été validée par l\'administrateur. La livraison ne peut pas être confirmée pour le moment. Veuillez patienter ou contacter un administrateur.');
        }
        ...
        if ($delivery->livreur_id === null) {
            ...
            return redirect()->route('livreur.deliveries.index')
                ->with('error', 'Cette livraison est déjà assignée à un autre livreur. Vous ne pouvez pas la valider.');
        }
        ...
        if ($order->agent?->phone) {
            ...
            return redirect()->route('livreur.deliveries.show', $delivery)
                ->with('success', 'Livraison validée par QR. Vous avez gagné 8 points. Commission de l\'agent calculée ce soir.')
    ...
    public function validateByCode(Request $request, Delivery $delivery, DeliveryService $deliveryService)
    {
        if ($delivery->livreur_id !== $request->user()->id) {
            return redirect()->route('livreur.deliveries.index')
                ->with('error', 'Cette livraison n\'est pas la vôtre. Vous ne pouvez pas la valider.');
        }
        ...
        if (! in_array($order->status, ['confirmed', 'preparing', 'delivering', 'delivered']) || $order->admin_validated_at === null) {
            return redirect()->route('livreur.deliveries.show', $delivery)
                ->with('error', 'Cette commande n\'a pas encore été validée par l\'administrateur. La livraison ne peut pas être confirmée pour le moment.');
        }
        ...
        if ($order->agent?->phone) {
            ...
            return redirect()->route('livreur.deliveries.show', $delivery)
                ->with('success', 'Livraison validée par le client. Vous avez gagné 8 points. Commission de l\'agent calculée ce soir.')
                ->with('whatsapp_link', $whatsappLink);
        }
    ...
    public function notifyClient(Request $request, Delivery $delivery)
    {
        if ($delivery->livreur_id !== $request->user()->id) {
            return redirect()->route('livreur.deliveries.index')
                ->with('error', 'Cette livraison n\'est pas la vôtre.');
        }
